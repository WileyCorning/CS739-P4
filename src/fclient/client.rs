...
use sah_lib::naive::encoding::pretty_print;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let two_x_plus_y: Vec<Example> = (vec![
        ...
        (vec![2,7],vec![11]),
        (vec![31,50],vec![112]),
    ]).into_iter().map(|(input,output)| Example{input,output}).collect();
...
async fn send( client:&mut FrontendClient<Channel>, examples: Vec<Example>) {
    ...
    match client.one_shot_solve(req).await {
        Ok(res) => {
            if let Some(a) = res.into_inner().outcome {
                if let Outcome::Solution(soln) = a {
